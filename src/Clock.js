...
const Clock = () => {
  ...
  const drawClock = () => {
    ...
    const hands = [
      ...
      { angle: minuteAngle, length: 140, width: 4, color: 'white', pointed: true },
    ...
    hands.forEach(({ angle, length, width, color, pointed, arrow }) => {
      ...
      if (arrow) {
        ...
        svg.append('polygon')
          .attr('points', `${x},${y} ${arrowHead1.x},${arrowHead1.y} ${arrowHead2.x},${arrowHead2.y}`)
          .attr('fill', color);
      } else if (pointed) {
        // Add the pointed end for hour and minute hands
        svg.append('polygon')
          .attr('points', `${x},${y} ${x - 5 * Math.sin(radians(angle + 2))},${y - 5 * Math.cos(radians(angle + 2))} ${x - 5 * Math.sin(radians(angle - 2))},${y - 5 * Math.cos(radians(angle - 2))}`)
          .attr('fill', color);
      }
    });
